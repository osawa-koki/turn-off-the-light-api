...
```bash
turn-off-the-light-api$ sam build
```
...
```bash
turn-off-the-light-api$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
turn-off-the-light-api$ sam local start-api
turn-off-the-light-api$ curl http://localhost:3000/
```
...
```bash
turn-off-the-light-api$ sam logs -n HelloWorldFunction --stack-name turn-off-the-light-api --tail
```
...
```bash
turn-off-the-light-api$ cd hello-world
...
```bash
sam delete --stack-name turn-off-the-light-api
```
